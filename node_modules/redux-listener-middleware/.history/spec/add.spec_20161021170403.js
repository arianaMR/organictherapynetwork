...
describe('middleware setup', () => {
  ...
  it('can mutate actions with rules', (done) => {
    ...
    const mw = listen();
    mw.createListener(listener).addRule('INCREMENT', payload => ({ foo: 'bar' }));
  ...
  it('can dispatch actions from the inside of a listener', (done) => {
    ...
    const listener = (type, payload, dispatch) => {
      setTimeout(() => dispatch(asyncAction(payload)));
    };

    const mw = listen();
    mw.createListener(listener).addRule('ADD_USER', payload => ({ firstName: 'Foo', lastName: 'Bar' }));
